...
!!! note "Deployment"
    The long compilation times described on this page are handled automatically in the
    Docker-based deployment — Rust and CUDA layers are cached in GHCR so CI skips
    recompilation when only Python code changes. See
    [Background → Docker & GHCR](docker-ghcr.md) for the full picture.
...
For Conda (on MSI), there is no automatic caching — if you need to rebuild, you re-run the full setup job. This is the main practical advantage of the Apptainer path: the Docker image is built once on GitHub's servers and cached on GHCR; MSI just downloads the finished result. See [Deployment → Apptainer](../apptainer-deployment.md) for how the full pipeline works, including how CI avoids recompiling Rust and CUDA on every push.
